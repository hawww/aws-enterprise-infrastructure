terraform {
  ...
  # aws s3api create-bucket --bucket enterprise-tfstate-us-east-1 --region REGION
  ...
  # aws dynamodb create-table --table-name enterprise-tflock --attribute-definitions AttributeName=LockID,AttributeType=S   --key-schema AttributeName=LockID,KeyType=HASH --provisioned-throughput ReadCapacityUnits=5,WriteCapacityUnits=5
  #
